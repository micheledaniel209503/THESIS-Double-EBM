function h_max = solve_hmax_for_params(params, h_vec, rc_vec)

    lh = numel(h_vec);
    lr = numel(rc_vec);

    Um_bot = zeros(lh, lr); Um_top = zeros(lh, lr); 
    Um_res = zeros(lh, lr); C_bot = zeros(lh, lr); 
    alpha  = zeros(lh, lr);

    for i = 1:lr
        [Um_bot(:, i), Um_top(:, i), Um_res(:, i), C_bot(:, i), alpha(:, i)] = ...
            double_EBM_RESERVOIR(h_vec, rc_vec(i), params);
    end
    ...
    % Alla fine:
    h_max = max(h_star);    % massimo h per quei params.hr e params.r2r
