...
count_fallbacks = sum(fallback_eq_h == 1);
...
V_min = 500; V_max = 8e3;
V_list = linspace(V_min, V_max, round(V_max/V_min)*5);  lv = numel(V_list);
...
for iv = 1:lv % for each value of V
    ...
    rc_prev = rc_star(iv);   % warm-start for the next V
end
